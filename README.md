...
Timer feature
-------------
Mount a volume (or bind mount) at `/daikin-web-control/data` to keep pending timers across container restarts.
Time worker output (aka logs) goes to `/var/log/daikin-timer.log` inside the container.

Run from Docker Hub
-------------------
`docker run -p 8000:80 -e UNIT_1_IP=192.168.1.11 -v daikin-data:/daikin-web-control/data e1025735/daikin-web-control`
